import { IQueryConfig, IQueryParams, IQueryResult, PrismaCountArgs, PrismaFindManyArgs, PrismaModelDelegate, PrismaNumberFilter, PrismaStringFilter, PrismaWhereConditions } from "../interface/queryBuilder.interface";
...
> {
    ...
    filter(): this {
        ...
        Object.keys(filterParams).forEach((key) => {
            ...
            // Handle bracket range syntax for numeric filters: price[gte]=100
            const bracketMatch = key.match(/^(.+)\[(.+)\]$/);
            if (bracketMatch) {
                const baseKey = bracketMatch[1];
                const operator = bracketMatch[2];

                const isAllowedField =
                    !filterableFields ||
                    filterableFields.length === 0 ||
                    filterableFields.includes(baseKey);

                if (!isAllowedField) {
                    return;
                }

                if (!queryWhere[baseKey]) {
                    queryWhere[baseKey] = {};
                    countQueryWhere[baseKey] = {};
                }

                const queryBase = queryWhere[baseKey] as Record<string, unknown>;
                const countBase = countQueryWhere[baseKey] as Record<string, unknown>;

                queryBase[operator] = this.parseFilterValue(value);
                countBase[operator] = this.parseFilterValue(value);
